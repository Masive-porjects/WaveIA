"""Mix Engine — per-stem routing (Paso 01: split → neutral bus → analysis).
...
stereo mono-compatible bus at NEUTRAL 0 dB gains (no DSP yet — pure
routing + summing), pads each stem to the longest one, writes the result
with ``write_output`` and produces per-stem analysis (LUFS / dynamic
range / spectral centroid) plus full-mix analysis (tempo / genre /
confidence).
...
def build_mix(session_id: str, input_path: str | Path) -> dict[str, Any]:
    ...
    # Read + resample every stem to the common bus rate.
    reads = [_read_stem_channels(stems[name]) for name in STEM_NAMES]
    target_sr = _common_sr([sr for _, sr in reads])
    bus_audio = [resample_audio(audio, stem_sr, target_sr) for audio, stem_sr in reads]
