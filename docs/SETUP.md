...
> Stack: Studio (Next.js :3000) · AudioMind (FastAPI :8000) · Convex (cloud).
...
|---|---|---|
| Python | **3.12** (requerido por audiomind) | el proyecto declara 3.12 |
...
./.venv/bin/pip install --retries 15 --timeout 90 -r apps/audiomind/requirements.txt
```

**Esperado:** `import librosa, pedalboard` sin errores.
...
# Python
cd apps/audiomind && ../.venv/bin/python -m pytest tests/ -q
```
...
| # | Pitfall | Detalle |
|---|---|---|
| 1 | Python 3.14 | el default de Homebrew puede romper deps del venv. Forzar `python3.12` |
| 2 | npm vs bun | el equipo migró a bun. `npm install` regenera package-lock y ensucia el repo. Usar `bun install` |
| 3 | `convex/_generated/` | NO se commitea (`.gitignore` de apps/studio). Se regenera con `npx convex dev` |
| 4 | `.env.local` | NO se commitea. `convex dev` lo genera con la URL del deployment propio |
| 5 | `@midimastering/agent` | apunta a `dist/` — sin `bun run build` previo, el studio no lo resuelve (TS2307) |
| 6 | typecheck global | los 3 errores de `convex/mastering.ts` son de Tomás (ciclo `api.d.ts` ↔ `mastering.ts`). No tocarlos sin avisarle |
| 7 | contrato live_params | `packages/contracts/live_params.schema.json` es la fuente de verdad. Si cambia: `packages/contracts/scripts/gen_types.sh` (regenera TS). Nunca editar tipos generados a mano |

---

## 8. Estado conocido del repo (septiembre 2026)

- ✅ studio tsc (salvo mastering.ts) + vitest + build · contrato live_params congelado · pytest audiomind verde
- ⚠️ Convex: deployment por dev — el del equipo llega cuando Tomás comparta el suyo
