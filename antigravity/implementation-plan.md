...
1. **Base de Trabajo**:
   - Cada fase se iniciará creando una **rama nueva basada en `staging`** (ejemplo: `feat/fase-0-limpieza-convex`, `feat/fase-1-features-i18n`, etc.).
2. **Sin Push Directo**:
   - Está **estrictamente prohibido hacer push directo a la rama `staging`**.
   - Toda integración a `staging` se realizará **exclusivamente a través de Pull Requests (PRs)** verificados.
...
4. **Ciclo por Fase**:
   - `git checkout staging` → `git pull origin staging` → `git checkout -b feat/fase-X-...`
   - Implementación + Pruebas locales (`lint`, `build`, `pytest`).
   - Commit semántico → Push de la rama feature → Creación del PR hacia `staging`.
